...
################################################################################
# EdgeRazor MaiProfile Training Pipeline — Qwen3-8B
################################################################################
# Usage: bash run.sh [w4] [layer_name]
...
case "${QUANT_ARG}" in
    "w4")
        ...
        RUN_NAME="maiprofile_${LAYER}_qwen3-8b_w4a8kv8"
        ;;
    *)
        echo "Usage: bash run.sh [w4] [layer_name]"
        exit 1
        ;;
esac

TRAIN_YAML="train"
TRAIN_VERSION="train_maiprofile_8b"
export QUANT_CONFIG
...
TRAIN_ROOT="${CODE_ROOT}/train_maiprofile_8b/${LAYER}_${QUANT_CONFIG}_${RUN_TAG}"
...
# Use MaiProfile 8B config class
sed -i.bak "s/= \"exp_for_sed\"/= \"${RUN_NAME}\"/" "${SRC_ROOT}/config_maiprofile_8b.py"
sed -i.bak "s/config = EdgeRazorTrainConfigFor[^(]*()/config = EdgeRazorTrainConfigForMaiProfile8B()/" "${SRC_ROOT}/main.py"

# Ensure main.py imports config_maiprofile_8b
if ! grep -q "from config_maiprofile_8b import" "${SRC_ROOT}/main.py"; then
    sed -i.bak "1i from config_maiprofile_8b import EdgeRazorTrainConfigForMaiProfile8B" "${SRC_ROOT}/main.py"
fi
...
echo "================================================================================"
echo "EdgeRazor MaiProfile Training Pipeline — Qwen3-8B"
echo "================================================================================"
...
echo "================================================================================"
echo "MaiProfile 8B Training Pipeline Completed!"
