...
################################################################################
# EdgeRazor MaiProfile Training Pipeline — Qwen3-4B
################################################################################
...
case "${QUANT_ARG}" in
    "w4")
        ...
        RUN_NAME="maiprofile_${LAYER}_qwen3-4b_w4a8kv8"
        ;;
    "w2.79")
        QUANT_CONFIG="W2.79A8KV8"
        TRAIN_YAML_DIR="w2.79a8kv8"
        RUN_NAME="maiprofile_${LAYER}_qwen3-4b_w2.79a8kv8"
        ;;
    "w1.58")
        QUANT_CONFIG="W1.58A8KV8"
        TRAIN_YAML_DIR="w1.58a8kv8"
        RUN_NAME="maiprofile_${LAYER}_qwen3-4b_w1.58a8kv8"
        ;;
    *)
        echo "Usage: bash run.sh [w4|w2.79|w1.58] [layer_name]"
        exit 1
        ;;
esac

TRAIN_YAML="train"
TRAIN_VERSION="train_maiprofile_4b"
export QUANT_CONFIG
...
TRAIN_ROOT="${CODE_ROOT}/train_maiprofile_4b/${LAYER}_${QUANT_CONFIG}_${RUN_TAG}"
...
# Use MaiProfile 4B config class
sed -i.bak "s/= \"exp_for_sed\"/= \"${RUN_NAME}\"/" "${SRC_ROOT}/config_maiprofile_4b.py"
sed -i.bak "s/config = EdgeRazorTrainConfigFor[^(]*()/config = EdgeRazorTrainConfigForMaiProfile4B()/" "${SRC_ROOT}/main.py"

# Ensure main.py imports config_maiprofile_4b
if ! grep -q "from config_maiprofile_4b import" "${SRC_ROOT}/main.py"; then
    sed -i.bak "1i from config_maiprofile_4b import EdgeRazorTrainConfigForMaiProfile4B" "${SRC_ROOT}/main.py"
fi
...
echo "================================================================================"
echo "EdgeRazor MaiProfile Training Pipeline — Qwen3-4B"
echo "================================================================================"
...
echo "================================================================================"
echo "MaiProfile 4B Training Pipeline Completed!"
