"""eval_layer3_seasonality.py — Evaluate seasonality accuracy (binary 0/1)."""
...
logger = logging.getLogger("edgerazor.eval_seasonality")

PROMPT_PATH = Path(__file__).parent / "prompts" / "layer3_eval_seasonality.md"
MODULE_NAME = "layer3_seasonality"
JSONL_FILENAME = "layer3_eval_seasonality.jsonl"
SUMMARY_FILENAME = "layer3_eval_seasonality_summary.json"


class Layer3SeasonalityEvaluator:
    ...
    def _build_user_prompt(interests):
        ...
        for i in interests:
            ...
            items.append({"interest_name": i.get("interest_name", ""), "evidence": evidence_list, "predicted_seasonality": seasonality})
        return json.dumps(items, ensure_ascii=False, indent=2)

    async def evaluate_user(self, user_id, date_str, interests):
        eval_interests = [i for i in interests if i.get("seasonality")]
        ...
        try:
            response_text, usage, elapsed, resp_len = await invoke_chat(
                ...
                response_format=None, caller="eval_l3_seasonality",
            )
            parsed = safe_json_loads(response_text)
        except Exception as exc:
            logger.warning("[%s][%s] seasonality eval failed: %s", user_id, date_str, exc)
        ...
        logger.info("[%s][%s] seasonality eval: %d/%d correct (%.1f%%)", user_id, date_str, correct, total, accuracy_rate * 100)
        return result
